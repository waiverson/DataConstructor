...
if __name__ == '__main__':
    ...
    # csv文件数据构造，导入到csv文件
    # Generater(CSVModel, processor=OutProcessor.csv_processor('csv')).produce()
    # salesforce数据导入到mongo
    # Generater(Salesforce, processor=OutProcessor.mongodb_processor('mongo')).produce()
    # csv文件构造导入到mongo
    Generater(CSVModel, out_processor=OutProcessor.mongodb_processor('mongo')).produce().load()
    # csv文件构造，导入到xls
    # Generater(CSVModel, processor=OutProcessor.xls_processor('xls')).produce()